...
const App: React.FC = () => {
  ...
  const [currentPage, setCurrentPage] = useState<CurrentPage>('main'); // State for page navigation
  ...
  const handleExportToPdf = async () => {
    ...
    try {
      const response = await fetch('/api/save-colors', {
        ...
        body: JSON.stringify({ colors: savedColors }),
      });
      if (!response.ok) {
        console.error('Failed to save colors to server:', response.statusText);
      } else {
        console.log('Colors saved to server successfully.');
      }
    } catch (error) {
    ...
    doc.text(pdfDate, 14, 30)

    const tableColumn = ['', `Cyan %`, `Magenta %`, `Yellow %`, `Black %`, `White %`];
    const tableRows: (string | number)[][] = [];

    savedColors.forEach(cmyk => {
      const displayPercentages = calculateAndFormatDisplayPercentages(cmyk);
      const rowData = ['', displayPercentages.c, displayPercentages.m, displayPercentages.y, displayPercentages.k, displayPercentages.w];
      tableRows.push(rowData);
    });

    (doc as any).autoTable({
      head: [tableColumn], body: tableRows, startY: 35, theme: 'grid',
      styles: { font: currentFont, fontSize: 10, cellPadding: 2, halign: 'center' },
      headStyles: { font: currentFont, fontStyle: 'bold', halign: 'center', valign: 'middle' },
      columnStyles: {
        0: { cellWidth: 20, halign: 'center' }, 1: { halign: 'right' }, 2: { halign: 'right' },
      ...
      willDrawCell: (data: any) => {
        if (data.section === 'head' && data.row.index === 0) {
          ...
          switch (data.column.index) {
            case 0: fillColor = [220, 223, 225]; textColor = [0, 0, 0]; break;
            case 1: fillColor = [0, 183, 235]; textColor = [0, 0, 0]; break;
            case 2: fillColor = [236, 0, 140]; textColor = [255, 255, 255]; break;
            case 3: fillColor = [255, 215, 0]; textColor = [0, 0, 0]; break;
            case 4: fillColor = [50, 50, 50]; textColor = [255, 255, 255]; break;
            case 5: fillColor = [240, 240, 240]; textColor = [0, 0, 0]; break;
          }
      ...
      didDrawCell: (data: any) => {
        if (data.section === 'body' && data.column.index === 0) {
          ...
          if (cmykColor) {
            ...
            doc.rect(data.cell.x + (data.cell.width - 10) / 2, data.cell.y + (data.cell.height - 10) / 2, 10, 10, 'F');
          }
        }
      },
    });
  ...
  if (currentPage === 'terms') {
    currentBackgroundStyle = initialPageBackground 
      ? { background: initialPageBackground } 
      : { backgroundColor: '#e2e8f0' }; // slate-200 as fallback
      ...
      <main className="bg-white p-[clamp(1rem,5vw,2.5rem)] rounded-[clamp(1rem,3vw,1.8rem)] flex flex-col items-center w-full">
        ...
        {savedColors.length > 0 && (
          <div className="flex flex-col items-center w-full mt-[clamp(0.75rem,3vw,1rem)]">
            <div className="flex flex-row flex-wrap justify-center items-center gap-2 w-full" aria-label={t('savedColorsListAriaLabel')}>
              {savedColors.map((savedCmyk, index) => {
                ...
                return (
                  <button
                    ...
                    aria-label={t('savedColorSwatchAriaLabel', { index: index + 1, c: savedCmyk.c, m: savedCmyk.m, y: savedCmyk.y, k: savedCmyk.k })}
                    title={t('savedColorSwatchAriaLabel', { index: index + 1, c: savedCmyk.c, m: savedCmyk.m, y: savedCmyk.y, k: savedCmyk.k })}
                  />);
              })}
            </div>
            <button
      ...
      <header className="mb-[clamp(1.5rem,5vh,3rem)] text-center w-full max-w-4xl relative">
        <div className="absolute top-0 right-0 z-20">
          ...
          <select
            ...
            className="bg-black border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-sm"
            aria-label={t('languageSelector_label')}
          >
            {(Object.keys(supportedLanguages) as LanguageCode[]).map((langCode) => (
              <option key={langCode} value={langCode} className="bg-black text-white">{supportedLanguages[langCode]}</option>
            ))}
          </select>
        </div>
      ...
      <footer className="mt-auto pt-[clamp(2rem,8vh,4rem)] text-center text-[clamp(0.75rem,2vw,0.875rem)] w-full max-w-2xl">
        <div className="mb-2 flex flex-wrap justify-center items-center gap-x-2">
          <button
            ...
            aria-label={t('footer_termsLinkAriaLabel')}
          >
            {t('footer_termsLink')}
          </button>
          ...
          <a
            ...
            className="text-white hover:text-slate-300 hover:underline focus:outline-none focus:underline"
            aria-label={t('footer_contactLinkAriaLabel')}
          >
            {t('footer_contactLink')}
          </a>
          <span className="text-white" aria-hidden="true">｜</span>
          <span className="text-white whitespace-nowrap">
            {t('footer_createrPrefix')}
            <a
              href="https://x.com/tetsuoadgj"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white hover:text-slate-300 hover:underline focus:outline-none focus:underline"
              aria-label={t('footer_createrLinkAriaLabel')}
            >
              {t('footer_createrName')}
            </a>
          </span>
        </div>
        <p className="text-white">{t('footer_copyright')}</p>
      </footer>
    </div>
  );
};
